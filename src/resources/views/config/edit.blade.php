<x-admin.layout.app>
    ...
    <form
        class="relative mt-7"
        method="POST"
        action="{{ route("LaravelBlock2Banner.config.update", $element->id) }}"
    >
        @csrf
        @method("patch")
        ...
        <x-admin.atoms.row>
            ...
            <x-admin.atoms.select name="layout" id="layout">
                @foreach (config("gmj.laravel_block2_banner_config.layouts") as $item)
        ...
        <div class="text-right">
            <x-admin.atoms.link
                href="{{ route('LaravelBlock2Banner.index', $element->id) }}"
            >
                Back
            </x-admin.atoms.link>
